

  def crust()
  crust = ["pan", "garlic butter","skin shavings"]
  end

  def size()
    size = ["small","medium","large"]
  end

  def drinks()
    drinks = ["coke","diet coke", "pepsi", "Mt. Dew", "Slurm"]
  end

  def saladsforwebapp()
    salads = ["cheff", "taco", "house", "Zoidberg shrimp"]
  end

  def wings()
    wings = ["dumpster wings", "buffallo, old new york style", "wong wings"]
  end
  def extra()
  extra= ["xcheese", "expep"]
  end

  def meats()
  meats= ["ham", "sausage", "pepperoni", "hamburger" , "anchovies" , "chicken", "racoon", "the deer you ran over with your car", "salami" , "prosciutto", "bacon" ]
  end

  def veggies()
  veggies = ["peppers", "onion", "tomato", "mushrooms", "spinach", "avocado" , "pesto", "pickles", "broccoli", "green mold from the dumpster out back" ]
  end

  def sauces()
  sauces = ["barbeque", "extra marinera", "alfredo", "cheeseburger pizzasauce", "Hummus", "Tapenade", "white"]
  end
